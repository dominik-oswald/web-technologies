...
(() => {
  ...
  /* ----------------------------------------------------------
     ...
     ---------------------------------------------------------- */
  document.querySelectorAll('.project-card__img[src]').forEach((vid) => {
  ...
  if (filterBar && cards.length) {
    ...
    if (archiveGridEl) {
      archiveGridEl.querySelectorAll('.archive-grid__section').forEach((sectionLi) => {
        const bar = sectionLi.querySelector('.archive-section-bar');
        if (!bar) return;
        bar.addEventListener('click', () => {
          const sec = sectionLi.dataset.archiveSection;
          if (!sec) return;
          applyFilter(sec, true);
        });
      });
    }
  }
  ...
  if (projectModal && archiveGrid && cards.length) {
    ...
    const openFromCard = (card) => {
      ...
      if (modalBody) {
        ...
        if (tplId) {
          ...
          voVideo.src = card.dataset.galleryVideo;
